-- R/HOOD Connections & Messaging System Database Schema
-- Run this in your Supabase SQL editor

-- 1. Create connections table for follow/follower relationships
...
-- 2. Create message_threads table for better message organization
...
-- 3. Update messages table to include thread_id and message status
ALTER TABLE messages ADD COLUMN IF NOT EXISTS thread_id UUID REFERENCES message_threads(id) ON DELETE CASCADE;
ALTER TABLE messages ADD COLUMN IF NOT EXISTS message_type VARCHAR(20) DEFAULT 'text' CHECK (message_type IN ('text', 'image', 'audio', 'file'));
...
CREATE INDEX idx_message_threads_participant_1 ON message_threads(participant_1);
CREATE INDEX idx_message_threads_participant_2 ON message_threads(participant_2);
...
-- 7. Create RLS Policies for message_threads
...
-- 8. Update existing messages RLS policies
...
-- 9. Create function to automatically create message threads
...
-- 10. Create trigger for automatic thread creation
CREATE TRIGGER create_message_thread_trigger
  BEFORE INSERT ON messages
  FOR EACH ROW
  EXECUTE FUNCTION create_message_thread();

-- 11. Create function to update unread counts
...
BEGIN
  ...
  IF NEW.sender_id = participant_1_id THEN
    UPDATE message_threads
...
-- 12. Create trigger for unread count updates
