...
// expo-file-system v19 (Expo SDK 54) replaced the default entry with a brand-new
// File/Directory API. The classic upload helpers — createUploadTask,
// FileSystemUploadType, FileSystemSessionType — now live ONLY in the "/legacy"
// entry. Importing from "expo-file-system" leaves them undefined, which caused
// "Cannot read property 'BINARY_CONTENT' of undefined".
...
import {
  ...
  prepareMixAudioForStreaming,
} from "./audioTranscode";
import { needsAacTranscode } from "./mixAudioFormat";
import { mixArtworkColumns } from "./mixArtworkColumns";
import {
  GENRE_OTHER,
  MIN_MIX_GENRES,
  resolveMixGenres,
...
export { GENRE_OTHER, MIN_MIX_GENRES, parseMixGenresForForm, resolveMixGenres } from "./mixGenres";
...
export function validateMixSubmission({
  ...
  if (
    genres.includes(GENRE_OTHER) &&
    !String(mixData.customGenre || "").trim()
  ) {
    return {
      ok: false,
      title: "Genre Required",
      message: "Enter the custom genre.",
    };
  }
  const resolvedGenres = resolveMixGenres(genres, mixData.customGenre);
  if (resolvedGenres.length < MIN_MIX_GENRES) {
    return {
      ok: false,
      title: "Genre Required",
      message: "Please select at least 1 genre for your mix.",
    };
  }
  return { ok: true };
}
...
export async function uploadMixSubmission({
  ...
  let playbackExt = fileExt;
  let tempAacUri = null;
  let uploadedFileSize = selectedFile?.size;

  try {
  // WAV/AIFF must become AAC before upload — iPhone cannot stream ~1 GB WAVs.
  ...
  if (selectedFile) {
    ...
    if (needsAacTranscode(fileExt)) {
      setProg(2);
      preparedAudio = await prepareMixAudioForStreaming({
        fileUri,
        fileExt,
        fileSize: selectedFile.size,
        mimeType: selectedFile.mimeType,
        onProgress: (fraction) => setProg(2 + Math.round(fraction * 8)),
      });
      if (preparedAudio.transcoded) {
        tempAacUri = preparedAudio.uri;
      }
    } else {
      preparedAudio = {
        uri: fileUri,
    ...
    playbackExt = preparedAudio.ext || fileExt;
    uploadedFileSize = preparedAudio.size || selectedFile.size;
  }

  // ── Run remaining independent work concurrently ────────────────────────────
  // The audio upload, artwork upload, duration probe and artist lookup don't
  // depend on each other. Previously they ran one after another, so the
  // duration probe (which loads the whole file via expo-av) and the artwork
  // upload were dead time tacked onto the audio upload. Running them in
  // parallel removes that overhead from the critical path without changing how
  // any individual step works.

  // 1) Audio upload (new file only). Drives the progress bar 10 → 75.
  let audioUploadPromise;
  if (selectedFile && preparedAudio) {
    const mixSlug = slugify(mixData.title || selectedFile.name);
    fileName = `${userId}/audio/${timestamp}_${mixSlug}_${randomStr}.${playbackExt}`;
    setProg(10);
    // Wrapped in withRetry to match uploadArtworkStreaming's reliability —
    // previously the large audio file (the one most likely to hit a
    // transient network blip on a slow upload) had no retry at all, unlike
    // the much smaller artwork upload.
    audioUploadPromise = withRetry(() =>
      uploadAudioFileStreaming({
        fileUri: preparedAudio.uri,
        fileName,
        contentType: preparedAudio.mimeType || "audio/mpeg",
        onProgress: (fraction) => setProg(10 + Math.round(fraction * 65)),
      })
    );
  } else if (editingMix) {
    audioUploadPromise = Promise.resolve({ publicUrl: editingMix.file_url });
  } else {
    audioUploadPromise = Promise.resolve(null);
  }

  // 2) Duration probe (reads the local file that will actually stream).
  const durationPromise = selectedFile
    ? resolveAudioDurationMillis({
        selectedFile: {
          ...selectedFile,
          uri: preparedAudio?.uri || selectedFile.uri,
          fileCopyUri: preparedAudio?.uri || selectedFile.fileCopyUri,
        },
        selectedFileDuration,
        Audio,
      })
    : Promise.resolve({ durationMillis: selectedFileDuration });

  // 3) Artwork upload (new artwork only) — runs alongside the audio upload.
  let artworkPromise = Promise.resolve(null);
  if (selectedArtwork) {
    const artworkSourceName =
      selectedArtwork.name ||
      selectedArtwork.fileName ||
    ...
    const mixSlug = slugify(
      mixData.title || selectedFile?.name || editingMix?.title || "mix"
    );
    const artworkFileName = `${userId}/artwork/${timestamp}_${mixSlug}_${randomStr}.${safeArtworkExt}`;
    artworkPromise = uploadArtworkStreaming({ selectedArtwork, artworkFileName });
  }

  // 4) Artist name (DB read).
  const artistPromise = fetchArtistName(userId);

  const [urlData, durationResult, artworkUpload, artistResult] = await Promise.all([
    audioUploadPromise,
    durationPromise,
    artworkPromise,
    artistPromise,
  ...
  let effectiveDurationMillis = durationResult?.durationMillis ?? selectedFileDuration;
  if (
    selectedFile &&
    effectiveDurationMillis &&
    effectiveDurationMillis !== selectedFileDuration &&
    typeof onDurationResolved === "function"
  ) {
    onDurationResolved(effectiveDurationMillis);
  }
  ...
  const { userProfile, artistName } = artistResult;

  const updateData = {
    title: mixData.title.trim(),
    description: mixData.description.trim() || null,
    genre: (() => {
      const resolved = resolveMixGenres(mixData.genres, mixData.customGenre);
      if (resolved.length) return resolved.join(",");
      return mixData.genre || "Electronic";
    })(),
  ...
  if (artworkUrl) {
    Object.assign(updateData, mixArtworkColumns(artworkUrl));
  } else if (editingMix && !selectedArtwork) {
    const existingArtwork =
      editingMix.artwork_url || editingMix.image_url || null;
    if (existingArtwork) {
      Object.assign(updateData, mixArtworkColumns(existingArtwork));
    }
  }
  ...
  if (editingMix) {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        ...mixArtworkColumns(finalArtworkUrl),
        play_count: 0,
        likes_count: 0,
        duration:
  ...
  return {
    ...
    await deleteLocalAudioFile(tempAacUri);
  }
}
