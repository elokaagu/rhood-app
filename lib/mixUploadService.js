 ...
 */
export async function cleanupFailedUpload({ fileName, artworkPublicUrl }) {
  if (fileName) {
    try {
  ...
  if (artworkPublicUrl) {
    try {
      const artworkPath = artworkPublicUrl.split("/mixes/")[1];
      if (artworkPath) {
...
export async function uploadMixSubmission({
  ...
  let artworkUrl = null;
  let artworkStoragePathForCleanup = null;
  ...
  if (selectedArtwork) {
    ...
    artworkUrl = await uploadArtworkBytes({
      artworkFileName,
      artworkData,
      contentType,
    });
    artworkStoragePathForCleanup = artworkUrl;
  }
  ...
  if (dbError) {
    ...
    await cleanupFailedUpload({
      fileName,
      artworkPublicUrl: artworkStoragePathForCleanup,
    });
    throw dbError;
  }
