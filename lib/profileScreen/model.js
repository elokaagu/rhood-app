...
export function mapGigsForProfile(gigsData) {
  ...
  return gigsData.slice(0, 5).map((gig) => ({
    id: gig.id,
    opportunityId: gig.opportunity_id || null,
    name: gig.name,
    venue: gig.venue,
    date: formatDateWithOrdinal(gig.event_date),
    price: profilePayLabel({ payment: gig.payment, payment_currency: gig.currency }),
    rating: Number(gig.dj_rating) > 0 ? Number(gig.dj_rating) : null,
...
export function formatApplicationStatusLabel(status) {
  ...
  if (key === "rejected") return "Not selected";
  if (key === "withdrawn") return "Withdrawn";
  return "Applied";
}

/** Pay only when the opportunity actually has compensation — never a dummy £0. */
export function profilePayLabel(opportunity) {
  const compensation =
    typeof opportunity?.compensation === "string"
      ? opportunity.compensation.trim()
      : "";
  if (compensation) return compensation;

  const payment = Number(opportunity?.payment);
  if (!Number.isFinite(payment) || payment <= 0) return null;

  const currency = String(
    opportunity?.payment_currency || opportunity?.currency || "GBP"
  ).toUpperCase();
  const symbol = currency === "USD" ? "$" : currency === "EUR" ? "€" : "£";
  return `${symbol}${Math.round(payment)}`;
}

export function mapApplicationRowForProfile(row, ratingStars = null) {
  const opp = row?.opportunities && typeof row.opportunities === "object"
    ? row.opportunities
    : {};
  ...
  return {
    id: row?.id,
    opportunityId: row?.opportunity_id || opp.id || null,
    name: opp.title || row?.name || "Opportunity",
    venue: opp.venue || opp.location || opp.city || row?.venue || "",
    date: formatDateWithOrdinal(opp.event_date || row?.event_date),
    price: profilePayLabel(opp) || profilePayLabel(row),
    rating: Number.isFinite(stars) && stars > 0 ? stars : null,
    status: statusKey,
    statusLabel: formatApplicationStatusLabel(statusKey),
    gigCompleted: Boolean(row?.gig_completed),
  };
}

/**
 * Recent Opportunities = applications the DJ submitted.
 * Recent Gigs = only those the brand/admin marked done (applications.gig_completed).
 */
export function splitProfileOpportunityActivity(applications, ratingsByApplicationId = {}) {
  const rows = Array.isArray(applications) ? applications : [];
  const recentOpportunities = rows.slice(0, 5).map((row) =>
    mapApplicationRowForProfile(row, ratingsByApplicationId[row.id])
  );
  const completed = rows.filter(
    (row) =>
      Boolean(row?.gig_completed) &&
      String(row?.status || "").toLowerCase() === "approved"
  );
  const recentGigs = completed.slice(0, 5).map((row) =>
    mapApplicationRowForProfile(row, ratingsByApplicationId[row.id])
  );
  return {
    recentOpportunities,
    recentGigs,
    gigsCompleted: completed.length,
  };
}
