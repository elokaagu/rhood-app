...
const USE_EDGE_FUNCTION = true; // Set to false to use direct OpenAI (for local dev only)
  ...
  if (USE_EDGE_FUNCTION) {
    try {
      ...
      if (error) {
        ...
        // Don't fallback to direct OpenAI - show helpful error instead
        return {
          text:
      ...
      console.error("Edge function exception:", error?.message || error);
