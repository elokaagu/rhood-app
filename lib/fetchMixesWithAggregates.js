...
export async function fetchMixesWithAggregates() {
  ...
  let listenSessionCountByMixId = {};
  if (mixIds.length > 0) {
    const { data: listenRows, error: listenError } = await supabase.rpc(
      "get_mix_listen_counts",
      { p_mix_ids: mixIds }
    );
    if (listenError) {
      if (__DEV__) {
        console.warn(
          "[fetchMixesWithAggregates] get_mix_listen_counts:",
          listenError.message || listenError
        );
      }
    } else if (Array.isArray(listenRows)) {
      listenSessionCountByMixId = listenRows.reduce((acc, row) => {
        if (!row?.mix_id) return acc;
        acc[row.mix_id] = Math.max(0, Number(row.session_count) || 0);
        return acc;
      }, {});
    }
  }
  ...
  const mixes = rows
    .map((mix) => {
      ...
      const sessionPlays = listenSessionCountByMixId[mix.id] ?? 0;
      const rowPlays = resolveMixPlayCount(mix);
      const plays = Math.max(rowPlays, sessionPlays, base.plays ?? 0);
      return {
        ...base,
        likeCount,
        plays,
