...
export const db = {
  ...
  async boostApplication(applicationId, boostDurationHours = 24, creditsCost = 10) {
    try {
      ...
      if (error) {
        const missingBoostRpc =
          error?.code === "PGRST202" ||
          error?.code === "42883" ||
          String(error?.message || "").includes("Could not find the function") ||
          String(error?.message || "").includes("boost_application");

        if (!missingBoostRpc) {
          throw error;
        }

        // Fallback path when RPC is not deployed yet in this environment.
        const {
          data: { user },
          error: authError,
        } = await supabase.auth.getUser();
        if (authError || !user?.id) {
          throw new Error("You must be signed in to boost an application.");
        }
        ...
        if (String(applicationRow.user_id) !== String(user.id)) {
          throw new Error("You can only boost your own applications.");
        }

        const isActiveBoost =
          applicationRow.is_boosted &&
          applicationRow.boost_expires_at &&
          new Date(applicationRow.boost_expires_at).getTime() > Date.now();
        if (isActiveBoost) {
          throw new Error("Application is already boosted.");
        }
        ...
        if (currentCredits < creditsCost) {
          throw new Error(
            `Insufficient credits. You need ${creditsCost} credits to boost this application.`
          );
        }

        const nextCredits = currentCredits - creditsCost;
        const boostExpiresAt = new Date(
          Date.now() + boostDurationHours * 60 * 60 * 1000
        ).toISOString();
        ...
        const { error: boostUpdateError } = await supabase
          ...
          .update({
            ...
            boost_credits_cost: creditsCost,
            updated_at: new Date().toISOString(),
          })
          .eq("id", applicationId)
          .eq("user_id", user.id);
