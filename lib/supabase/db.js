...
export const db = {
  ...
  async getUserProfilePublic(userId) {
    ...
    if (!fromView.error && fromView.data) {
      return sanitizePublicProfile(fromView.data);
    }
  ...
  async finalizeDjMembership(inviteCode) {
    const { data, error } = await supabase.rpc("finalize_dj_membership", {
      p_invite_code: inviteCode ? String(inviteCode).trim() : null,
    });
    if (error) throw error;
    return data || "pending";
  },
  ...
  /**
   * Studio DJ invite codes: redeem_dj_invite_code / invite_code_used.
   * Friend DJ codes still go through processReferral (credits). Missing RPCs
   * fall back to the live membership_status, or approved if the column isn't live.
   */
      ...
      if (!error) {
        ...
        try {
          return normalizeMembershipStatus(
            await this.finalizeDjMembership(code)
          );
        } catch (finalizeError) {
          if (!isMissingRpcError(finalizeError, "finalize_dj_membership")) {
            console.warn("Membership finalize failed:", finalizeError);
          }
        }
      } else {
