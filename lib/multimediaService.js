...
export const multimediaService = {
  ...
  async pickVideo() {
    try {
      ...
      // Pick video with thumbnail generation enabled
      const result = await ImagePicker.launchImageLibraryAsync({
      ...
      // Generate thumbnail from video if not provided by picker
      let thumbnailUri = asset.thumbnailUri;

      // If thumbnailUri is not provided, try to generate one from the video
      // Note: expo-image-picker should provide thumbnailUri on iOS, but we'll try to generate if missing
      if (!thumbnailUri && asset.uri) {
        try {
          // Try generating thumbnail using our helper
          thumbnailUri = await this.generateVideoThumbnail(asset.uri);
          if (thumbnailUri) {
            console.log("📸 Generated video thumbnail:", thumbnailUri);
          }
        } catch (thumbError) {
  ...
  async pickAudio() {
    try {
      ...
      // Note: filename will be set by user prompt in MessagesScreen
      return {
        type: "audio",
        uri: file.uri,
        filename: file.name, // Default to original name, can be overridden
        size: file.size,
  ...
  /**
   * Generate a thumbnail frame from a video file
   * Note: On iOS, expo-image-picker automatically provides thumbnailUri
   * On Android, we may need to use a native module or fallback
   * @param videoUri - URI of the video file
   * @returns Promise<string|null> - URI of the generated thumbnail image or null
   */
  async generateVideoThumbnail(videoUri) {
    try {
      console.log("📸 Attempting to generate thumbnail from video:", videoUri);

      // expo-image-picker should already provide thumbnailUri on iOS
      // For a more robust solution, we could use expo-video-thumbnails if available
      // or implement a native module. For now, we'll rely on the picker's thumbnailUri
      // and return null if we need to generate one manually (which would require native code)

      // Note: expo-image-picker's launchImageLibraryAsync for videos
      // should automatically generate and return thumbnailUri on supported platforms

      return null; // Let the picker handle it
    } catch (error) {
  ...
  async uploadToStorage(selectedMedia) {
    try {
      ...
      // Upload thumbnail for videos if available
