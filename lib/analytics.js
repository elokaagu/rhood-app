...
export async function trackScreenView(screenName, properties = {}) {
  try {
    ...
    // GA4 (modular API) — use logEvent; logScreenView is deprecated in RN Firebase v22+
    if (analyticsInstance) {
      const { logEvent } = require("@react-native-firebase/analytics");
      console.log(`📊 [GA4] Tracking screen view: ${screenName}`, properties);
      const params = {
        firebase_screen: String(screenName).slice(0, 100),
        firebase_screen_class: String(screenName).slice(0, 100),
      };
      Object.entries(properties || {}).forEach(([k, v]) => {
        if (v == null) return;
        if (typeof v === "string" || typeof v === "number" || typeof v === "boolean") {
          params[String(k).slice(0, 40)] = v;
        }
      });
      await logEvent(analyticsInstance, "screen_view", params);
