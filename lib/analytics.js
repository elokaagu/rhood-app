 ...
 * Should be called once when app starts, after ATT on iOS.
 */
export async function initAnalytics({ trackingAllowed = true } = {}) {
  try {
    await Promise.all([
      initMixpanel({ trackingAllowed }),
    ...
    try {
      ...
      if (user) {
        await setAnalyticsUser(user.id, {
          name: user.user_metadata?.dj_name || user.user_metadata?.full_name || undefined,
        });
      }
    } catch (error) {
