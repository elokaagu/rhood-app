...
class AIMatchmakingEngine {
  constructor(apiKey, provider = 'openai') {
    ...
    this.claudeModelId = aiConfig.defaults.claudeModel;
  }
...
Provide detailed reasoning for each match, considering both obvious fits and unexpected but valuable opportunities. Be specific about why each match works and what makes it special.`;
  ...
  getUserPrompt({
    ...
    const scenarioLine = scenario
      ? `\n## MATCHING SCENARIO\nUse this lens when ranking: **${String(scenario).replace(/_/g, ' ')}**.\n`
      : '';
...
`;
  ...
  generateDetailedReasons(match) {
    ...
    return {
      musical_fit: r.includes('genre') ? 'High' : 'Medium',
      skill_alignment: r.includes('skill') ? 'Perfect' : 'Good',
      venue_compatibility: r.includes('venue') ? 'Excellent' : 'Good',
      career_impact: r.includes('career') ? 'High' : 'Medium',
    };
  }
