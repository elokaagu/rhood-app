...
class LockScreenControls {
  ...
  async showLockScreenNotification(track) {
    ...
    if (Platform.OS === "android" && Notifications) {
      ...
      // On iOS, use notifications to ensure lock screen controls appear
      // expo-av handles Now Playing automatically, but notifications ensure visibility
      await this.updateIOSNotification(true);
    }
  }
  ...
  async setPlaybackState(isPlaying, positionMillis, durationMillis) {
    ...
    if (Platform.OS === "android" && Notifications) {
      ...
      await this.updateIOSCategory();
      await this.updateIOSNotification(false);
    }
  }
  ...
  async updateIOSNotification(forceUpdate) {
    if (!(Platform.OS === "ios" && Notifications)) {
      return;
    }
    if (!this.currentTrack) {
    ...
    if (this.notificationId) {
      try {
        ...
        console.log("⚠️ Failed to dismiss old iOS notification:", error);
      }
    }
    ...
    try {
      ...
      console.log("❌ Failed to schedule iOS media notification:", error);
    }
  }
