...
class LockScreenControls {
  ...
  async ensureInitialized() {
    ...
    // IMPORTANT: On iOS, react-native-track-player manages the audio session internally
    // We should NOT call Audio.setAudioModeAsync() on iOS when using TrackPlayer
    // as it causes audio session conflicts ("could not fetch audio session category" errors)
    // Only configure audio mode on Android, or on iOS if TrackPlayer is not available
    if (Platform.OS === "android") {
      try {
        ...
        console.log(
          "✅ Audio mode configured for Android lock screen controls"
        );
      } catch (error) {
      ...
      // The iosCategory: "playback" in TrackPlayer.updateOptions() handles this
  ...
  async showLockScreenNotification(track) {
    ...
    // iOS: react-native-track-player handles native controls automatically
    if (Platform.OS === "android" && Notifications) {
  ...
  async hideLockScreenNotification() {
    // Android: Dismiss MediaStyle notification
    // iOS: react-native-track-player handles clearing native controls automatically
