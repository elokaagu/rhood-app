...
try {
  ...
  console.log(
    "⚠️ Notifications module not available (Expo Go):",
    error.message
  );
  Notifications = null;
}
...
class LockScreenControls {
  ...
  setupNotificationHandler() {
    if (!Notifications) {
      console.log(
        "⚠️ Notifications not available - skipping notification handler setup"
      );
      return;
    }
  ...
  async showLockScreenNotification(trackData) {
    if (!Notifications) {
      console.log(
        "⚠️ Notifications not available - skipping lock screen notification"
      );
      return;
    }
    ...
    try {
      ...
      if (Platform.OS === "ios") {
        console.log(
          "🔒 iOS: expo-av provides automatic Now Playing controls for lock screen"
        );
        return;
      }
      ...
      console.log(
        "🔒 Android: Lock screen notification shown for:",
        trackData.title
      );
    } catch (error) {
  ...
  async setupMediaCategories() {
    if (!Notifications) {
      console.log(
        "⚠️ Notifications not available - skipping media categories setup"
      );
      return;
    }
