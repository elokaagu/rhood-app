...
class LockScreenControls {
  ...
  async showLockScreenNotification(trackData) {
    try {
      ...
      // For iOS, expo-av handles lock screen controls automatically
      // We just need to ensure audio mode is configured (done in App.js)
      if (Platform.OS === "ios") {
        console.log("🔒 Lock screen controls enabled via expo-av for:", trackData.title);
        return;
      }

      // For Android, use expo-notifications for MediaStyle controls
      if (!Notifications) {
        console.log(
  ...
  async updateLockScreenNotification(progress = null) {
    // For iOS, expo-av handles updates automatically
    if (Platform.OS === "ios") {
      return;
    }
