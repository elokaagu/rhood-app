...
// Conditionally import Notifications (may not be available in Expo Go)
let Notifications;
try {
  Notifications = require("expo-notifications");
  console.log("✅ Notifications module available");
} catch (error) {
  console.log("⚠️ Notifications module not available (Expo Go):", error.message);
  Notifications = null;
}
...
class LockScreenControls {
  ...
  async showLockScreenNotification(trackData) {
    if (!Notifications) {
      console.log("⚠️ Notifications not available - skipping lock screen notification");
      return;
    }
    ...
    try {
      ...
      // Don't create notifications - expo-av handles lock screen controls automatically on iOS
      // Android will need platform-specific implementation
      if (Platform.OS === "ios") {
        console.log("🔒 iOS: expo-av provides automatic Now Playing controls for lock screen");
        return;
      }
      
      // For Android, create media notification
      ...
      console.log("🔒 Android: Lock screen notification shown for:", trackData.title);
    } catch (error) {
  ...
  async updateLockScreenNotification(progress = null) {
    // iOS: expo-av handles Now Playing automatically
    if (Platform.OS === "ios") {
      return;
    }
    
    if (!Notifications || !this.currentTrack || !this.notificationId) {
      return;
    }
  ...
  async setupMediaCategories() {
    if (!Notifications) {
      console.log("⚠️ Notifications not available - skipping media categories setup");
      return;
    }
