...
class LockScreenControls {
  ...
  async showLockScreenNotification(track) {
    ...
    if (Platform.OS === "ios") {
      await nowPlayingInfo.setNowPlayingInfo({
        ...
        positionMillis: this.positionMillis || 0,
  ...
  async setPlaybackState(isPlaying, positionMillis, durationMillis) {
    ...
    // iOS: Update Now Playing metadata with current playback state
    if (Platform.OS === "ios" && this.currentTrack) {
      await nowPlayingInfo.setNowPlayingInfo({
        title: this.currentTrack.title || "R/HOOD Mix",
        artist: this.currentTrack.artist || "Unknown Artist",
        album: this.currentTrack.genre || "R/HOOD",
        image: this.currentTrack.image || null,
        durationMillis: this.durationMillis || 0,
        positionMillis: this.positionMillis || 0,
        isPlaying: this.isPlaying,
      });
    }
