...
export function transformOpportunityRow(opp, userLocation) {
  ...
  const isOngoing = !opp.event_end_time;
  // Studio stores the start as event_date (timestamptz). Prefer that over
  // event_start_time so the app shows the same clock as the listing.
  const eventDateIsTimestamp =
    typeof opp.event_date === "string" && /T/i.test(opp.event_date);
  let startTimeRaw = eventDateIsTimestamp
    ? opp.event_date
    : opp.event_start_time ??
      opp.start_time ??
      opp.event_time ??
      null;
  let endTimeRaw = isOngoing
    ? null
  ...
  const formattedTime = isOngoing
    ? "Ongoing"
    : formatOpportunityTime(startTimeRaw, endTimeRaw);
  const compensationText =
    typeof opp.compensation === "string" ? opp.compensation.trim() : "";
  const formattedCompensation = compensationText
    ? compensationText
    : formatOpportunityCompensation(
        opp.payment,
  ...
  return {
    ...
    distance,
    distanceFormatted,
    date: isOngoing && formattedDate !== "TBD"
