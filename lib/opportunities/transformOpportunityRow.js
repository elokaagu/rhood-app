...
export function transformOpportunityRow(opp, userLocation) {
  const timeZone = opp.event_timezone ?? opp.timezone ?? null;
  const formattedDate = formatOpportunityDate(opp.event_date, timeZone);
  const isOngoing = !opp.event_end_time;
  // Studio stores the start as event_date (timestamptz) in event_timezone.
