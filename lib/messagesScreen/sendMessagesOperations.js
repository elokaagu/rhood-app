...
export function buildDirectBase(threadId, userId) {
  return {
    thread_id: threadId,
    sender_id: userId,
  };
...
export function mapMediaToDirectRow(baseMessageData, mediaItem) {
  const durationMs =
    Number.isFinite(Number(mediaItem?.duration)) && Number(mediaItem.duration) > 0
  ...
  return {
    ...
    metadata: durationMs ? { duration_millis: durationMs } : null,
  };
}

export function mapMediaToGroupRow(communityId, userId, mediaItem) {
  const durationMs =
    Number.isFinite(Number(mediaItem?.duration)) && Number(mediaItem.duration) > 0
      ? Number(mediaItem.duration)
      : null;
 ...
 */
import { assertCanMessageUser } from "../moderation";
...
export async function sendIndividualChatMessages({
  ...
  messageContent,
  mediaArray,
  canMessage = assertCanMessageUser,
}) {
  try {
    await canMessage(djId);
  } catch (error) {
    return {
      ok: false,
      error,
  ...
  if (mediaArray.length > 0) {
    ...
    if (error) {
      ...
      return {
        ok: false,
        error,
        // messageContent (if any) already committed successfully above —
        // don't hand it back for the composer to repopulate, or a retry
        // re-inserts the same text as a visible duplicate. Only the media
        // portion actually needs retrying.
        rollback: { messageContent: "", mediaArray },
        textAlreadySent: !!messageContent,
      };
    }
  }
  ...
  if (mediaArray.length > 0) {
    ...
    if (error) {
      ...
      return {
        ok: false,
        error,
        // Same reasoning as sendIndividualChatMessages: messageContent (if
        // any) already committed above — don't repopulate it for retry.
        rollback: { messageContent: "", mediaArray },
        textAlreadySent: !!messageContent,
      };
    }
