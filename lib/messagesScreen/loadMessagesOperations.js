...
import { hasApplicationConversationAccess } from "../applicationConversation";
import { isAcceptedConnectionStatus } from "../connectionStatusUtils";
...
export async function loadIndividualThreadState({
  ...
  //
  // Filtered client-side (not via a `.not(...)` query clause) so this keeps
  // working even before add-delete-message-for-me.sql's column exists —
  // `|| []` defaults to "hidden from no one" when the column is absent.
  const messages = (data || [])
    .filter((msg) => !(msg.deleted_for_user_ids || []).includes(userId))
    .slice()
    .reverse()
  ...
  const connected = isAcceptedConnectionStatus(connection?.connection_status);
  const viaApplication = connected
    ? false
    : await hasApplicationConversationAccess(djId);
  ...
  return {
    ...
    payload: {
      ...
      canCompose: connected || viaApplication,
...
export async function loadGroupThreadState({
  ...
  const groupMessages = await db.getGroupMessages(communityId, userId);
