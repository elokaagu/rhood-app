  ...
  try {
    ...
    if (error) {
      const { data: recentUsers } = await supabase
        ...
        .order("created_at", { ascending: false })
        .limit(fetchLimit);
      setPopularDJs(filterDirectoryDjs(recentUsers).slice(0, DISCOVER_DJ_SLICE));
      return;
    }
    setPopularDJs(filterDirectoryDjs(popularUsers).slice(0, DISCOVER_DJ_SLICE));
  } catch (error) {
...
export async function loadNearbyOpportunitiesImpl(ctx) {
  ...
  try {
    ...
    const cityMatched = (opportunitiesData || []).filter((opp) =>
      citiesShareSameCity(cityNeedle, opp.city)
    );
...
export async function loadNearbyDJsImpl(ctx) {
  ...
  try {
    ...
    const filtered = filterDirectoryDjs(nearbyUsers)
      .filter((u) => !connectedUserIds.has(u.id))
      .filter((u) => citiesShareSameCity(cityNeedle, u.city || u.location))
...
export async function loadDiscoverDJsImpl(ctx) {
  ...
  try {
    ...
    const formattedDiscoverUsers = filterDirectoryDjs(recommendedUsers).map((user) => {
