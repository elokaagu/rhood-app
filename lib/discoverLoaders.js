...
import { formatOpportunityDate } from "./formatters";
import { isMissingColumnError } from "./membership";
...
const DJ_PROFILE_SELECT_BASE =
  "id, email, dj_name, profile_image_url, city, location, genres, credits, gigs_completed, created_at, full_name, username, is_verified";
const DJ_PROFILE_SELECT = `${DJ_PROFILE_SELECT_BASE}, membership_status`;

async function selectDirectoryDjs(buildQuery) {
  let result = await buildQuery(DJ_PROFILE_SELECT);
  if (result.error && isMissingColumnError(result.error)) {
    result = await buildQuery(DJ_PROFILE_SELECT_BASE);
  }
  return result;
}
...
export async function loadPopularDJsImpl(ctx) {
  ...
  try {
    ...
    if (error) {
      const { data: recentUsers } = await selectDirectoryDjs((columns) =>
        supabase
          .from("user_profiles")
          .select(columns)
          .not("dj_name", "is", null)
...
export async function loadNearbyOpportunitiesImpl(ctx) {
  ...
  try {
    ...
    const orFilter = aliases
      ...
      .join(",");
    const nearbySelect =
      "id, title, venue, city, event_date, event_timezone, genre, payment, payment_currency, compensation, image_url";
    const nearbySelectNoTz =
      "id, title, venue, city, event_date, genre, payment, payment_currency, compensation, image_url";
    let { data: opportunitiesData, error } = await supabase
      .from("opportunities")
      .select(nearbySelect)
      .eq("is_active", true)
      ...
      .limit(20);
    if (error && /event_timezone/i.test(error.message || "")) {
      const retry = await supabase
        .from("opportunities")
        .select(nearbySelectNoTz)
        .eq("is_active", true)
        .or(orFilter || `city.ilike.%${cityNeedle}%`)
        .gte("event_date", new Date().toISOString().split("T")[0])
        .order("event_date", { ascending: true })
    ...
    const transformed = cityMatched.slice(0, 10).map((opp) => ({
      ...
      date: opp.event_date
        ? formatOpportunityDate(opp.event_date, opp.event_timezone)
        : "TBD",
      genre: opp.genre || "Electronic",
      compensation: (typeof opp.compensation === "string" && opp.compensation.trim())
        ? opp.compensation.trim()
        : opp.payment
