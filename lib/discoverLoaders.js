...
/** Fetch extra rows so brand accounts (filtered client-side) do not empty the carousel. */
const DISCOVER_DJ_FETCH_MULTIPLIER = 3;

function filterNonBrandProfiles(rows) {
  return (rows || []).filter((p) => !isBrandAccount(p?.email));
}
...
export async function loadPopularDJsImpl(ctx) {
  ...
  try {
    ...
    const { data: popularUsers, error } = await supabase
      .from("user_profiles")
      .select("id, email, dj_name, profile_image_url, city, genres, credits, gigs_completed, created_at")
      .not("dj_name", "is", null)
    ...
    if (error) {
      const { data: recentUsers } = await supabase
        .from("user_profiles")
        .select("id, email, dj_name, profile_image_url, city, genres")
        .not("dj_name", "is", null)
        .order("created_at", { ascending: false })
        .limit(fetchLimit);
      setPopularDJs(filterNonBrandProfiles(recentUsers).slice(0, DISCOVER_DJ_SLICE));
      return;
    }
    setPopularDJs(filterNonBrandProfiles(popularUsers).slice(0, DISCOVER_DJ_SLICE));
  } catch (error) {
...
export async function loadNearbyDJsImpl(ctx) {
  ...
  try {
    ...
    const { data: nearbyUsers, error } = await supabase
      .from("user_profiles")
      .select("id, email, dj_name, profile_image_url, city, genres, credits, gigs_completed, created_at, full_name, username, is_verified")
      .not("dj_name", "is", null)
    ...
    const filtered = filterNonBrandProfiles(nearbyUsers)
