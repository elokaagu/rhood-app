...
export const matchmaking = {
  ...
  async setDJPreferences(userId, preferences) {
    try {
      ...
      const preferencesData = Object.entries(preferences).map(([type, value]) => ({
        user_id: userId,
  ...
  async generateMatches(userId, limit = 20) {
    try {
      ...
      const matches = data.map(match => {
        const opportunity = opportunities.find(opp => opp.id === match.opportunity_id);
        return {
          ...match,
  ...
  async getMatchmakingAnalytics(userId) {
    try {
      ...
      const analytics = {
        ...
        acceptedApplications: applications.data?.filter(a => a.status === 'accepted').length || 0,
        ...
        averagePerformanceRating: performanceHistory.length > 0 ?
          performanceHistory.reduce((sum, p) => sum + (p.rating || 0), 0) / performanceHistory.length : 0
      };
