...
function normalizeMatch(match) {
  ...
  return {
    ...match,
    match_score: Number.isFinite(Number(match.match_score))
...
function normalizeVenue(venue) {
  ...
  return {
    ...venue,
    rating: Number.isFinite(Number(venue.rating)) ? Number(venue.rating) : 0,
  };
}
...
function normalizePerformance(performance) {
  ...
  return {
    ...performance,
    rating: Number.isFinite(Number(performance.rating))
...
export const matchmaking = {
  ...
  async setDJPreferences(userId, preferences) {
    try {
      ...
      const preferenceTypes = preferenceEntries.map(([type]) => type);
      const preferencesData = preferenceEntries.map(
        ([type, value]) => ({
          user_id: userId,
      ...
      // Upsert to avoid delete-then-insert gaps.
      const { data, error } = await supabase
        .from("dj_preferences")
        .upsert(preferencesData, { onConflict: "user_id,preference_type" })
        .select();

      if (error) throw error;

      // Remove preferences that are no longer provided.
      ...
      if (preferenceTypes.length === 0) {
        ...
        ({ error: deleteError } = await supabase
          ...
          .delete()
          .eq("user_id", userId)
          .not("preference_type", "in", `(${preferenceTypes.map((t) => `"${t}"`).join(",")})`));
      }
      if (deleteError) throw deleteError;

      return (data || []).map((item) => ({
        ...item,
        importance_score: Number.isFinite(Number(item.importance_score))
          ? Number(item.importance_score)
          : 1,
      }));
    } catch (error) {
  ...
  async generateMatches(userId, limit = 20) {
    try {
      ...
      const matches = data.map((match) => {
        const opportunity = opportunityById.get(match.opportunity_id) || null;
        return {
  ...
  async applyToOpportunity(userId, opportunityId, message = "") {
    try {
      ...
      const { data: hasMixes, error: mixError } = await supabase
        .from("mixes")
        .select("id")
        .eq("user_id", userId)
        .eq("is_public", true)
      ...
      if (mixError) {
        console.error("Error checking user mixes:", mixError);
        throw new Error("Failed to check user mixes");
      }

      if (!hasMixes || hasMixes.length === 0) {
        throw new Error(
  ...
  async getPerformanceHistory(userId, limit = 50) {
    try {
      ...
      if (error) throw error;
      return (data || []).map(normalizePerformance);
    } catch (error) {
