...
export async function reverseGeocode(latitude, longitude) {
  ...
  try {
    ...
    if (addresses && addresses.length > 0) {
      ...
      const city =
        address.city || address.subAdministrativeArea || address.region || null;
      return city || null;
    }
    return null;
  } catch (error) {
...
/**
 * GPS → city label for the shared location picker (Edit Profile / onboarding).
 * Prefers Google Geocoding when a key is present; falls back to expo-location.
 */
export async function resolveCurrentCityLabel() {
  const location = await getCurrentLocation();
  if (!location) return null;

  try {
    const { getGooglePlacesApiKey } = await import("./googlePlacesConfig");
    const { reverseGeocodeLatLngWithGoogle } = await import("./googlePlacesClient");
    const apiKey = getGooglePlacesApiKey();
    if (apiKey) {
      const googleCity = await reverseGeocodeLatLngWithGoogle(
        location.latitude,
        location.longitude,
        apiKey
      );
    ...
    if (__DEV__) {
      console.warn("[resolveCurrentCityLabel] Google geocode:", error);
    }
  }
