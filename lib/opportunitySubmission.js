...
function buildEventTimestamps(eventDate, startTime, endTime) {
  ...
  const toUtcIso = (time, dayOffset) => {
    if (!time) return null;
    const [hh, mm] = time.split(":").map(Number);
    return new Date(y, m - 1, d + dayOffset, hh, mm, 0).toISOString();
  };

  const startTs = toUtcIso(startTime, 0);
  const rollsOverToNextDay = !!startTime && !!endTime && endTime < startTime;
  const endTs = toUtcIso(endTime, rollsOverToNextDay ? 1 : 0);
