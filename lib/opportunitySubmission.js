 ...
 *
 * Builds an explicit UTC instant (.toISOString(), always "...Z") rather than
 * a bare "YYYY-MM-DDTHH:MM:00" string. A zone-less string is ambiguous — a
 * TIMESTAMPTZ column parses it using the *database session's* timezone, not
 * the device's, so "19:00" typed by a DJ in London could get silently
 * reinterpreted as 19:00 in whatever timezone the DB session happens to be
 * in, then re-displayed shifted by however many hours separate the two —
 * exactly the "picked 7pm, shows up as 1:30am" bug. Using local Date
 * components (new Date(y, m, d, h, min) — interpreted in the *device's* own
 * timezone, matching what the DJ actually picked on their wall clock) and
 * converting that to an explicit UTC instant removes the ambiguity: Postgres
 * always parses an offset/Z-suffixed string as that exact instant,
 * regardless of its own session timezone setting.
 *
 ...
 * timestamp before the start timestamp). Date's own day-overflow arithmetic
 * (day 32 → the 1st/2nd of the next month) handles month/year boundaries.
 */
/**
 * Combine the posted calendar date + clock in `event_timezone` into a
 * timestamptz instant — same as Studio's parseEventDateTime.
 */
function buildEventTimestamps(eventDate, startTime, endTime, timeZone) {
  const toIso = (date, time) => {
    if (!date || !time) return null;
    const instant = parseEventDateTime(date, time, timeZone);
    if (Number.isNaN(instant.getTime())) return null;
    return instant.toISOString();
  };

  const startTs = toIso(eventDate, startTime);
  const endDate =
    startTime && endTime && endTime < startTime
      ? nextCalendarDay(eventDate)
      : eventDate;
  const endTs = toIso(endDate, endTime);
...
export function buildOpportunityRow(form, userId, organizerFallback) {
  ...
  const timezone = trimmed(form.timezone) || resolveTimeZone();
  const { startTs, endTs } = buildEventTimestamps(
    eventDate,
    startTime,
    endTime,
    timezone
...
export async function submitOpportunity(form, userId, organizerFallback) {
  ...
  if (error) {
    ...
    if (error.message?.includes("event_timezone")) {
      const fallbackRow = { ...buildOpportunityRow(form, userId, organizerFallback) };
      delete fallbackRow.event_timezone;
