...
export function isDirectoryReadyDj(profile) {
  ...
  if (profile.is_verified === true) return hasDirectoryName(profile) && hasDirectoryPhoto(profile);
  const membership = String(profile.membership_status || "").toLowerCase();
  if (membership === "pending" || membership === "rejected") return false;
