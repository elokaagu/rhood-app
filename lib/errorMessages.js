...
import { isUncompressedRemoteAudio } from "./mixAudioFormat";
...
export function getUserFriendlyError(error, defaultMessage = "Something went wrong. Please try again.") {
  const { lower: errorMsg, code: errorCode } = normalizeError(error);

  // Specific credential/auth errors first
  ...
  if (
    ...
    ((errorCode === "23505" || errorMsg.includes("duplicate key value")) && errorMsg.includes("email")) ||
  ...
  // Specific network errors before broad matches
  ...
  if (
    (errorMsg.includes("file") && errorMsg.includes("format")) ||
    (errorMsg.includes("file") && errorMsg.includes("type")) ||
 ...
 */
export function getAudioErrorMessage(error, audioUrl) {
  if (isUncompressedRemoteAudio(audioUrl)) {
    return "This mix is an uncompressed WAV, which iPhone can't stream reliably. Re-upload the same WAV — we'll convert it for playback.";
  }

  const { lower: errorMsg } = normalizeError(error);
