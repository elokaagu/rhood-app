...
export async function uploadOnboardingProfileImage(imageUri, mimeType) {
  ...
  // This upload runs pre-email-confirmation (anon role, no auth.uid()), so
  // storage RLS can't scope it to an owner — an unguessable random token in
  // the path is the only defense against another anon caller overwriting an
  // in-progress signup's photo by guessing/enumerating nearby timestamps.
