...
export const connectionsService = {
  ...
  async searchUsers(query, limit = 20) {
    try {
      const { data, error } = await supabase
        .from('user_profiles')
        .select('*')
        .or(`dj_name.ilike.%${query}%,full_name.ilike.%${query}%,city.ilike.%${query}%`)
        .limit(limit);
  ...
  async getRecommendedUsers(limit = 10) {
    try {
      ...
      // Get users from same city or with similar genres
      const { data, error } = await supabase
        .from('user_profiles')
        .select('*')
        .neq('id', user.id)
        .not('id', 'in', `(SELECT following_id FROM connections WHERE follower_id = '${user.id}')`)
        .order('created_at', { ascending: false })
        .limit(limit);
  ...
  async getMessageThreads() {
    try {
      ...
      return data.map(thread => {
        const otherParticipant = thread.participant_1 === user.id 
          ? thread.participant_2_profile 
          : thread.participant_1_profile;
        
        const unreadCount = thread.participant_1 === user.id 
        ...
        return {
          ...
          otherParticipant,
          unreadCount,
          lastMessageSender: thread.last_message_sender_id === user.id ? 'me' : 'other'
        };
      });
    } catch (error) {
