...
export const connectionsService = {
  ...
  async getRecommendedUsers(limit = 10, currentUserId = null) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .neq("id", userId)
        .order("created_at", { ascending: false })
        .limit(limit * 3); // Get more to account for filtering out brand accounts
  ...
  async getMessages(threadId, limit = 50) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .limit(Math.min(limit, 1000)); // Ensure we don't exceed Supabase's max limit
  ...
  async getAllCommunities() {
    try {
      ...
      if (authError) {
        console.warn("getAllCommunities: unable to resolve auth user", authError);
      }
      ...
      // One membership query per chunk (was: 2×N round-trips via isCommunityMember + getUser each time).
      ...
      if (userId && communityIds.length > 0) {
        for (let i = 0; i < communityIds.length; i += COMMUNITY_IN_CHUNK) {
          ...
          if (membershipError) {
            console.warn(
              "getAllCommunities: batch membership error",
              membershipError
            );
          } else {
      ...
      // Bounded batched queries for preview avatars (was: N separate queries).
      ...
      if (communityIds.length > 0) {
        try {
          for (let i = 0; i < communityIds.length; i += COMMUNITY_IN_CHUNK) {
            ...
            if (avatarError) {
              console.warn(
                "getAllCommunities: batch avatars error",
                avatarError
              );
            } else {
          ...
          console.warn(
            "getAllCommunities: unexpected batch avatar error",
            avatarErr
          );
        }
      }
      ...
      const transformedCommunities = communities.map((community) => {
        ...
        return {
          ...
          recentActivity: formatRelativeTime(
