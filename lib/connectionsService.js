  ...
  async getRecommendedUsers(limit = 10) {
    try {
      ...
      if (filteredData && filteredData.length > 0) {
        console.log("🔍 First recommended user data:", filteredData[0]);
        console.log("🔍 Profile image URL field:", filteredData[0].profile_image_url);
        console.log("🔍 Image URL field:", filteredData[0].image_url);
      }
    ...
    try {
      ...
      const transformedCommunities = await Promise.all(
        (data || []).map(async (community) => {
          let isMember = false;
          if (userId) {
            isMember = await this.isCommunityMember(community.id);
          }
          ...
          try {
            ...
            if (memberError) {
              console.warn(
                "getAllCommunities: error fetching member avatars",
                memberError
              );
            } else {
              memberAvatars =
            ...
            console.warn(
              "getAllCommunities: unexpected error fetching member avatars",
              avatarError
            );
          }
