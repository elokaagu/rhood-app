...
const isBrandAccount = (email) => {
  ...
  // List of brand account email patterns
  const brandPatterns = [
    /^team@/i,
    /^support@/i,
    /^info@/i,
  ...
  // Check if email matches any brand pattern
...
export const connectionsService = {
  ...
  async searchUsers(query, limit = 20) {
    try {
      ...
      const filteredData = (data || []).filter((userProfile) => {
        return !isBrandAccount(userProfile.email);
      });
  ...
  async getRecommendedUsers(limit = 10, currentUserId = null) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .neq("id", userId)
        .order("created_at", { ascending: false })
        .limit(Math.min(safeLimit * 3, MAX_QUERY_LIMIT)); // Get more to account for filtering out brand accounts
