...
import { listBlockedUserIds, listBlockedUserIdsEitherWay, assertCanMessageUser } from "./moderation";

/** PostgREST `.in()` lists can hit URL limits; chunk large id sets. */
const COMMUNITY_IN_CHUNK = 80;
...
function normalizeQueryLimit(limit, fallback = 20) {
  const n = Number(limit);
  if (!Number.isFinite(n) || n <= 0) return fallback;
  return Math.min(Math.floor(n), MAX_QUERY_LIMIT);
}

function escapeSearchTerm(input) {
  const raw = String(input || "").trim();
  if (!raw) return "";
  // Escape PostgREST filter special characters used in `.or(...)` expressions.
...
function formatRelativeTime(timestamp) {
  ...
  if (Number.isNaN(target)) return "Recently active";
  const diffMinutes = Math.floor((now - target) / (1000 * 60));
  ...
  return `Active ${diffWeeks} wk${diffWeeks === 1 ? "" : "s"} ago`;
}

async function fetchJoinedCommunityIds(userId, communityIds) {
  const joinedIds = new Set();
  if (!userId || !Array.isArray(communityIds) || communityIds.length === 0) {
    return joinedIds;
  }
  for (let i = 0; i < communityIds.length; i += COMMUNITY_IN_CHUNK) {
    const chunk = communityIds.slice(i, i + COMMUNITY_IN_CHUNK);
    ...
    if (membershipError) {
      console.warn("getAllCommunities: batch membership error", membershipError);
      continue;
    }
    (membershipRows || []).forEach((row) => {
      if (row?.community_id) joinedIds.add(row.community_id);
    });
  }
  return joinedIds;
}

async function fetchCommunityPreviewAvatars(communityIds) {
  const avatarsByCommunity = new Map();
  ...
  for (let i = 0; i < communityIds.length; i += COMMUNITY_IN_CHUNK) {
    ...
    `
      ...
      .limit(250);
    if (avatarError) {
      console.warn("getAllCommunities: batch avatars error", avatarError);
      continue;
    }
    for (const row of avatarRows || []) {
      const cid = row.community_id;
      const url = row.user_profiles?.profile_image_url;
      if (!cid || !url) continue;
      if (!avatarsByCommunity.has(cid)) {
        avatarsByCommunity.set(cid, []);
      }
      const arr = avatarsByCommunity.get(cid);
      if (arr.length < 5) arr.push(url);
    }
  }
  return avatarsByCommunity;
}

function mapCommunityForListItem(community, joinedIds, avatarsByCommunity) {
  const communityLocation =
    community.location && String(community.location).trim()
      ? String(community.location).trim()
      : null;
  const locationLabel = communityLocation || "Global";
  return {
    id: community.id,
    name: community.name,
    ...
    isTrending: (community.member_count || 0) > 1000,
    recentActivity: formatRelativeTime(community.updated_at || community.created_at),
    featuredContent: `Welcome to ${community.name}!`,
    communityImage:
      "https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=400&h=200&fit=crop",
    memberAvatars: avatarsByCommunity.get(community.id) || [],
...
export const connectionsService = {
  ...
  async searchUsers(query, limit = 20) {
    try {
      ...
      const safeLimit = normalizeQueryLimit(limit, 20);
      if (!safeQuery) return [];
      const { data, error } = await supabase
        .from("user_profiles")
        .select("*")
        .or(`dj_name.ilike.%${safeQuery}%,city.ilike.%${safeQuery}%`)
        .limit(Math.min(safeLimit * 2, MAX_QUERY_LIMIT)); // Get more to account for filtering

      if (error) throw error;

      const blockedIds = await listBlockedUserIds();
      const filteredData = excludeUserIds(
        filterDirectoryDjs(data),
        blockedIds
      ).slice(0, safeLimit);
   ...
   * @param {number} limit
   * @param {string} [currentUserId] - If set, skips auth.getUser() (caller already has session).
   */
  async getRecommendedUsers(limit = 10, currentUserId = null) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .neq("id", userId)
        .order("created_at", { ascending: false })
        .limit(Math.min(safeLimit * 5, MAX_QUERY_LIMIT)); // Extra rows after directory filters

      if (error) throw error;

      const blockedIds = await listBlockedUserIds();
      const filteredData = excludeUserIds(filterDirectoryDjs(data), blockedIds);

      // Return filtered users for discovery (connections will be shown in connections tab)
      // We'll let the UI handle showing connection status
      return filteredData.slice(0, safeLimit);
    } catch (error) {
  ...
  async getMessageThreads() {
    try {
      ...
      const blockedIds = await listBlockedUserIdsEitherWay();
      const visible = (data || []).filter((thread) => {
        const otherId =
  ...
  async getOrCreateThread(otherUserId) {
    try {
      const user = await requireCurrentUser();
      await assertCanMessageUser(otherUserId);
  ...
  async sendMessage(receiverId, content, messageType = "text") {
    try {
      const user = await requireCurrentUser();
      await assertCanMessageUser(receiverId);
  ...
  async getMessages(threadId, limit = 50) {
    try {
      const safeLimit = normalizeQueryLimit(limit, 50);
      ...
      const { data, error } = await supabase
        ...
        .limit(Math.min(safeLimit, 1000)); // Ensure we don't exceed Supabase's max limit
    ...
    return supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "connections",
          filter: `or(follower_id.eq.${userId},following_id.eq.${userId},user_id_1.eq.${userId},user_id_2.eq.${userId})`,
        },
        callback
      )
      .subscribe();
  ...
  async getGroupMessages(communityId, limit = 50) {
    try {
      ...
      return normalized.reverse(); // Reverse to show oldest first
    } catch (error) {
  ...
  async getAllCommunities() {
    try {
      ...
      const [joinedIds, avatarsByCommunity] = await Promise.all([
        fetchJoinedCommunityIds(userId, communityIds),
        fetchCommunityPreviewAvatars(communityIds),
      ]);

      return communities.map((community) =>
        mapCommunityForListItem(community, joinedIds, avatarsByCommunity)
      );
    } catch (error) {
