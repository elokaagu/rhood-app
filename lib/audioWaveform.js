...
export function getDurationSeconds(mixOrDuration) {
  if (typeof mixOrDuration === "number" && Number.isFinite(mixOrDuration)) {
    return Math.max(1, mixOrDuration);
  }

  const mix = mixOrDuration && typeof mixOrDuration === "object" ? mixOrDuration : {};

  if (typeof mix.durationSeconds === "number" && Number.isFinite(mix.durationSeconds)) {
    return Math.max(1, mix.durationSeconds);
  }
  if (typeof mix.duration === "number" && Number.isFinite(mix.duration)) {
    return Math.max(1, mix.duration);
  }
  if (typeof mix.durationMillis === "number" && Number.isFinite(mix.durationMillis)) {
    return Math.max(1, Math.round(mix.durationMillis / 1000));
  }

  return DEFAULT_DURATION_SECONDS;
}

function buildWaveformCacheKey(mix, points) {
  if (mix?.id) return `id:${mix.id}:p:${points}`;
  if (mix?.audioUrl) return `url:${mix.audioUrl}:p:${points}`;
  return `shape:${getDurationSeconds(mix)}:${mix?.genre || "default"}:p:${points}`;
}
...
export function generateSimpleWaveform(duration, points = 16) {
  const waveform = [];
  const seed = getDurationSeconds(duration); // Use duration as seed for consistency
  ...
  for (let i = 0; i < points; i++) {
    ...
    waveform.push(clampWaveHeight(base + variation));
  }
...
export function generateGenreWaveform(duration, genre = 'electronic', points = 16) {
  const waveform = [];
  const normalizedDuration = getDurationSeconds(duration);
  ...
  for (let i = 0; i < points; i++) {
    ...
    let height;
    const randomA = seededRandom(normalizedDuration + i * 13 + genre.length * 7);
    const randomB = seededRandom(normalizedDuration + i * 29 + genre.length * 11);
    ...
    if (buildups && position > 0.6 && position < 0.75) {
      ...
      // Drop section
      height = 80 + (randomA * 20);
    } else {
      // Normal variation
      const baseHeight = 30 + (intensity * 40);
      const variation = (randomB - 0.5) * variability * 40;
      height = baseHeight + variation;
    }
    
    waveform.push(clampWaveHeight(height));
  }
...
/**
 * Generate waveform from actual audio file (Web only, via AudioContext)
 * NOTE: This requires the audio file to be accessible and CORS-enabled
 * @param {string} audioUrl - URL to audio file
 * @param {number|Object} durationOrMix - Duration or mix object for deterministic fallback
 * @param {number} points - Number of waveform points
 * @returns {Promise<number[]>} - Array of waveform heights (0-100)
 */
export async function generateWaveformFromAudio(audioUrl, durationOrMix, points = 16) {
  const fallbackDuration = getDurationSeconds(durationOrMix);
  try {
    if (Platform.OS !== "web") {
      return generateSimpleWaveform(fallbackDuration, points);
    }
    ...
    for (let i = 0; i < points; i++) {
      ...
      waveform.push(clampWaveHeight(average * 200));
    }
    
    return waveform;
  } catch (error) {
    if (__DEV__) {
      console.warn("Error generating waveform from audio:", error);
    }
...
export async function getMixWaveform(mix, points = 16) {
  const duration = getDurationSeconds(mix);
  const cacheKey = buildWaveformCacheKey(mix, points);
  ...
  if (mix.waveform && Array.isArray(mix.waveform) && mix.waveform.length > 0) {
    waveformCache.set(cacheKey, mix.waveform);
    return mix.waveform;
  }
  
  // Try to generate from audio file on web only; falls back deterministically.
  if (mix.audioUrl && mix.audioUrl.startsWith('http')) {
    try {
      const waveform = await generateWaveformFromAudio(mix.audioUrl, mix, points);
      ...
      if (__DEV__) {
        console.log("Could not generate waveform from audio, using deterministic fallback");
      }
    }
  }
  ...
  if (mix.genre) {
    const waveform = generateGenreWaveform(duration, mix.genre, points);
    waveformCache.set(cacheKey, waveform);
    return waveform;
  }
  
  // Final fallback to simple waveform
  const waveform = generateSimpleWaveform(duration, points);
  waveformCache.set(cacheKey, waveform);
  return waveform;
}
