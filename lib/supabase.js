...
export const db = {
  ...
  async getLastMessageForConnection(userId, connectedUserId) {
    try {
      console.log(`🔍 getLastMessageForConnection: userId=${userId}, connectedUserId=${connectedUserId}`);
      ...
      console.log(`🔍 Last message for thread ${threadId}:`, data ? "Found" : "Not found");
      return data || null;
    } catch (error) {
  ...
  async getLastMessagesForAllConnections(userId) {
    try {
      ...
      console.log("🔍 Getting last messages for connections:", connections.length);
      ...
      const lastMessagesPromises = connections.map(async (connection) => {
        try {
          ...
          console.error("Error getting last message for connection:", connection.connected_user_id, error);
          return {
            connectionId: connection.connected_user_id,
            lastMessage: null,
          };
