  ...
  async getAllConversationParticipants(userId) {
    try {
      ...
      const [profilesRes, connectionsRes] = await Promise.all([
        ...
        supabase
          .from("connections")
          .select("id, user_id_1, user_id_2, status, connection_status")
          .or(`user_id_1.eq.${userId},user_id_2.eq.${userId}`),
      ]);
      ...
      (connectionsRes.data || []).forEach((c) => {
        ...
        const status = (c.status || c.connection_status || "").toString().trim().toLowerCase();
        if (["accepted", "approved", "connected"].includes(status)) {
          connectionByOtherId.set(other, c);
        }
      });
