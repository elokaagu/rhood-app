...
export const db = {
  ...
  async getUserProfile(userId) {
    const { data, error } = await supabase
      .from("user_profiles")
      .select(
        "id, dj_name, full_name, first_name, last_name, email, bio, location, city, genres, profile_image_url, phone, show_email, show_phone, primary_mix_id, credits, created_at, updated_at, username, is_verified, gigs_completed"
      )
      .eq("id", userId)
      .single();
  ...
  async getUserProfilePublic(userId) {
    const { data, error } = await supabase
      .from("user_profiles")
      .select(
        "id, dj_name, full_name, first_name, last_name, email, bio, location, city, genres, profile_image_url, phone, show_email, show_phone, primary_mix_id, created_at, updated_at, username, is_verified, gigs_completed"
      )
      .eq("id", userId)
      .single();
  ...
  async getUnreadGroupMessageCount(communityId, userId) {
    try {
      // Get the last time user viewed this community
      ...
      // Count messages after last view
      const { count, error } = await supabase
        .from("community_posts")
        .select("*", { count: "exact", head: true })
        .eq("community_id", communityId)
