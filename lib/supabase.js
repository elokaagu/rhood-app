...
export const db = {
  ...
  async getUserConnections(userId, status = "accepted") {
    ...
    return connections.map(conn => {
      const connectedUserId = conn.user_id_1 === userId ? conn.user_id_2 : conn.user_id_1;
      const connectedUser = users?.find(user => user.id === connectedUserId);
      
      return {
        id: conn.id,
        connected_user_id: connectedUserId,
        connected_user_name: connectedUser?.dj_name || connectedUser?.full_name || 'Unknown User',
