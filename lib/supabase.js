...
export const db = {
  ...
  async getUserProfile(userId) {
    const { data, error } = await supabase
      .from("user_profiles")
      .select(
        "id, dj_name, full_name, first_name, last_name, email, bio, location, city, genres, image_url, phone, show_email, show_phone, primary_mix_id, credits, created_at, updated_at"
      )
      .eq("id", userId)
      .single();
  ...
  async getUserProfilePublic(userId) {
    const { data, error } = await supabase
      .from("user_profiles")
      .select(
        "id, dj_name, full_name, first_name, last_name, email, bio, location, city, genres, image_url, phone, show_email, show_phone, primary_mix_id, created_at, updated_at"
      )
      .eq("id", userId)
      .single();
