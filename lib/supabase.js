...
export const auth = {
  ...
  async signInWithApple() {
    try {
      ...
      // 1) Try without nonce first to see if that works
      console.log("🔐 Attempting Apple Sign-In without nonce...");

      // 2) Request Apple ID token without nonce
      const credential = await AppleAuthentication.signInAsync({
        requestedScopes: [
          AppleAuthentication.AppleAuthenticationScope.FULL_NAME,
          AppleAuthentication.AppleAuthenticationScope.EMAIL,
        ],
        // No nonce for now - let's see if this works
      });
      ...
      // 3) Sign in with Supabase using the ID token (without nonce)
      console.log("🔐 Sending to Supabase without nonce:", {
