        ...
        if (accessToken) {
          ...
          try {
            ...
            if (profileError && profileError.code === "PGRST116") {
              // No profile found, create one from Google data
              ...
              if (createError) {
                ...
                const { data: verifyProfile, error: verifyError } =
                  await supabase
                    .from("user_profiles")
                    .select("*")
                    .eq("id", sessionData.user.id)
...
Overall: ${allChecksPass ? '✅ ALL CHECKS PASS' : '❌ VALIDATION FAILED'}`;
        ...
        Alert.alert("Apple Sign-In Debug", debugMessage, [
          ...
          { text: "Cancel", style: "cancel", onPress: () => { throw new Error("User cancelled due to validation failure"); } }
        ]);
