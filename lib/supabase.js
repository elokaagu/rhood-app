...
export const auth = {
  ...
  async signInWithApple() {
    try {
      ...
      // 1) Make a fresh RAW nonce (ASCII, 32+ chars)
      function makeRawNonce(len = 32) {
        const bytes = globalThis.crypto.getRandomValues(new Uint8Array(len));
        return Array.from(bytes, b => b.toString(16).padStart(2, "0")).join(""); // hex
      }

      // 2) Hash -> base64url (what Apple puts into the ID token's `nonce` claim)
      ...
      const rawNonce = makeRawNonce(32);
