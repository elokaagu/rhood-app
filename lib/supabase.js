...
export const auth = {
  ...
  async signInWithApple() {
    try {
      ...
      // 1) Create a RAW nonce (no unicode, 32+ chars, ASCII/hex)
      const rawNonce = await Crypto.getRandomBytesAsync(32).then((bytes) => {
        // Convert to hex string - this is the RAW nonce
      ...
      // 2) Request Apple ID token with RAW nonce
      // Recent Expo AppleAuthentication expects rawNonce (it hashes internally)
      const credential = await AppleAuthentication.signInAsync({
        requestedScopes: [
          AppleAuthentication.AppleAuthenticationScope.FULL_NAME,
          AppleAuthentication.AppleAuthenticationScope.EMAIL,
        ],
        nonce: rawNonce, // Pass RAW nonce to Apple
      });
      ...
      try {
        ...
        console.log("🔍 JWT Claims nonce:", claims.nonce);
        console.log(
          "🔍 Expected SHA256 of rawNonce:",
      ...
      // Always pass the RAW nonce to Supabase (never hashed)
