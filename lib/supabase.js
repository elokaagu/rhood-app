...
export const db = {
  ...
  async getUserProfile(userId) {
    const { data, error } = await supabase
      .from("user_profiles")
      .select(
        "id, dj_name, full_name, first_name, last_name, email, bio, location, city, genres, profile_image_url, phone, show_email, show_phone, primary_mix_id, credits, created_at, updated_at"
      )
      .eq("id", userId)
      .single();
  ...
  async getUserProfilePublic(userId) {
    const { data, error } = await supabase
      .from("user_profiles")
      .select(
        "id, dj_name, full_name, first_name, last_name, email, bio, location, city, genres, profile_image_url, phone, show_email, show_phone, primary_mix_id, created_at, updated_at"
      )
      .eq("id", userId)
      .single();
  ...
  async createConnection(userId1, userId2) {
    ...
    // If connection exists, return it instead of creating a new one
    ...
    const { data, error } = await supabase
      ...
      .insert([
        {
          user_id_1: userId1 < userId2 ? userId1 : userId2, // Ensure consistent ordering
          user_id_2: userId1 < userId2 ? userId2 : userId1,
          status: "pending",
          initiated_by: userId1, // The user who initiated the connection
          created_at: new Date().toISOString(),
        },
      ])
      .select()
