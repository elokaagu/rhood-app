...
export const auth = {
  ...
  async signInWithApple() {
    ...
    try {
      ...
      // Forensic logging
      console.log('[APPLE] rawNonce len/preview', rawNonce.length, rawNonce.slice(0,6), rawNonce.slice(-6));
      console.log('[APPLE] hashedNonce len/preview', hashedNonce.length, hashedNonce.slice(0,6), hashedNonce.slice(-6));
      ...
      try {
        ...
        console.log('[APPLE] token.nonce === hashed?', claims.nonce === hashedNonce);
        ...
        console.warn('⚠️ Could not decode token for verification:', decodeError.message);
      }

      console.log("✅ Apple credential received");
      console.log('[SB call] nonce len/preview', rawNonce.length, rawNonce.slice(0,6), rawNonce.slice(-6));
      ...
      console.log('[SB payload nonce len/preview]', payload.nonce?.length, payload.nonce?.slice(0,6), payload.nonce?.slice(-6));

      const { data, error } = await supabase.auth.signInWithIdToken(payload);
      console.log('[SB sdk result]', { dataPresent: !!data?.session, error: error?.message });
      ...
      if (error) {
        ...
        try {
          const res = await fetch(
            "https://jsmcduecuxtaqizhmiqo.supabase.co/auth/v1/token?grant_type=id_token",
            {
              ...
              headers: {
                ...
                apikey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpzbWNkdWVjdXh0YXFpemhtaXFvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTczNDUwNDIsImV4cCI6MjA3MjkyMTA0Mn0.CxQDVhiWf8qFf0SB0evnqniyMYUttpwF3ThlpB8dfso",
              },
              body: JSON.stringify({
                provider: "apple",
          ...
          if (res.status === 200) {
            console.log("✅ REST call succeeded - SDK was dropping/modifying nonce");
