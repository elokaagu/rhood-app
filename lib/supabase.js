...
export const auth = {
  ...
  async signInWithApple() {
    ...
    try {
      ...
      console.log(
        "🔐 Generated hashed nonce length:",
        hashedNonceB64Url.length
      );
      
      // TEMPORARY: Use fixed nonce for debugging
      const rawNonce = 'abc123-TEST-nonce-ONLY-for-debug';
      const hashedNonceB64Url = await sha256Base64url(rawNonce);
      console.log("🔧 USING FIXED NONCE FOR DEBUGGING:", rawNonce);
      ...
      // Detailed Supabase call logging
      console.log("[SB call] nonce len/preview", rawNonce.length, rawNonce.slice(0,6), rawNonce.slice(-6));
      console.log("[SB call] token len", credential.identityToken.length);
      ...
      console.log("[SB payload]", payload, Object.keys(payload));
      ...
      if (error) {
        ...
        // Try direct REST call to GoTrue to bypass SDK
        console.log("🔄 Attempting direct REST call to GoTrue...");
        try {
          ...
          if (res.status === 200) {
            console.log("✅ REST call succeeded - SDK likely dropped the nonce");
            return { user: json.user, session: json };
          } else {
            console.log("❌ REST call also failed - nonce/hash flow issue");
          }
        } catch (restError) {
