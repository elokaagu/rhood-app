  ...
  async signInWithGoogle(isSignupFlow = false) {
    try {
      ...
      if (result.type === "success") {
        ...
        if (code) {
          console.log("📝 Received OAuth code, Supabase should handle session automatically");
          // Supabase will automatically exchange the code when we get the session
          // Just return and let the auth state change handle it
          ...
          // If session not available yet, wait a moment for Supabase to process
          ...
          console.warn("⚠️ No session found after OAuth callback with code");
        }
...
export const db = {
  ...
  async getLastMessagesForAllConnections(userId) {
    try {
      ...
      console.log(
        "🔍 Getting last messages for connections:",
        connections.length
      );

      // Get last message for each connection
      const lastMessagesPromises = connections.map(async (connection) => {
        try {
          const lastMessage = await this.getLastMessageForConnection(
            userId,
            connection.connected_user_id
          );
          return {
            connectionId: connection.connected_user_id,
            lastMessage: lastMessage,
          };
        } catch (error) {
          console.error(
            "Error getting last message for connection:",
            connection.connected_user_id,
            error
          );
          return {
      ...
      results.forEach(({ connectionId, lastMessage }) => {
        if (lastMessage) {
          // Use the connection ID that matches what the UI expects
          lastMessagesMap[connectionId] = {
            content: lastMessage.content,
            timestamp: lastMessage.created_at,
            senderId: lastMessage.sender_id,
            senderName:
              lastMessage.sender?.full_name ||
              lastMessage.sender?.dj_name ||
              "Unknown",
  ...
  async findOrCreateIndividualMessageThread(user1Id, user2Id) {
    try {
      ...
      // First, let's check if there are ANY threads between these users (both orders)
      ...
      console.log(`⚠️ No existing thread found. Error:`, findError?.message);
      ...
      if (findError && findError.code !== "PGRST116") {
        ...
        console.log(
          `🔍 No existing thread found (PGRST116) for users ${id1} and ${id2}`
        );
      }

      // If no thread found, create a new one
      console.log(`📝 Creating new thread for users: ${id1}, ${id2}`);
      ...
      if (createError) {
        ...
        if (createError.code === "23505") {
          ...
          console.log(`✅ Refetched existing thread: ${reFetchedThread.id}`);
