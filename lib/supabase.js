    ...
    try {
      ...
      const lastMessagesPromises = threads.map(async (thread) => {
        try {
          ...
          if (error) {
            // Handle infinite recursion error in RLS policies
            if (error.code === "42P17" || error.message?.includes("infinite recursion")) {
              console.warn(
                "⚠️ RLS policy recursion detected for thread:",
                thread.id,
                "- This is a database policy issue that needs to be fixed"
              );
              ...
              // Return null gracefully - this thread will be skipped
              return null;
            }
