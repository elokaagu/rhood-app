...
export const db = {
  ...
  async getAllConversationParticipants(userId) {
    try {
      ...
      console.log(
        "🔍 Found threads, getting participants:",
        threads.length
      );
      ...
      const participantsPromises = threads.map(async (thread) => {
        try {
          ...
          if (!finalOtherUserId) {
            console.log("⚠️ Could not determine other user for thread:", thread.id);
            return null;
          }

          // Get the other user's profile
          const otherUserProfile = await this.getUserProfilePublic(finalOtherUserId);

          if (!otherUserProfile) {
            console.log("⚠️ Could not fetch profile for user:", finalOtherUserId);
            return null;
          }
          ...
          if (error) {
            console.error("Error fetching last message for thread:", thread.id, error);
          }

          return {
            userId: finalOtherUserId,
            name: otherUserProfile.dj_name || otherUserProfile.full_name || "Unknown User",
            ...
            lastMessage: data ? {
              ...
              senderName: data.sender?.full_name || data.sender?.dj_name || "Unknown",
  ...
  async getLastMessagesForAllConnections(userId) {
    try {
      ...
      console.log(
        "🔍 Getting last messages for threads:",
        threads.length
      );
