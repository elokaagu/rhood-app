    ...
    try {
      ...
      const lastMessagesPromises = threads.map(async (thread) => {
        try {
          ...
          console.log("✅ Found message for thread:", thread.id, "-", data.content?.substring(0, 30));
  ...
  async findOrCreateIndividualMessageThread(user1Id, user2Id) {
    try {
      ...
      if (anyThreads && anyThreads.length > 0) {
        ...
        console.log(`✅ Using existing thread (oldest of ${anyThreads.length}): ${oldestThread.id}`);
        
        // If there are multiple threads, log a warning
        if (anyThreads.length > 1) {
          console.warn(`⚠️ WARNING: Found ${anyThreads.length} threads for this user pair! Using oldest: ${oldestThread.id}`);
          console.warn(`⚠️ Other thread IDs:`, anyThreads.slice(1).map(t => t.id));
        }
      ...
      console.log(`📝 No thread found, creating new one for users: ${id1}, ${id2}`);
      ...
      if (createError) {
        ...
        if (createError.code === "23505") {
          console.warn("⚠️ Race condition: Thread already exists, fetching it...");
