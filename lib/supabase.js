  ...
  async hasUserUploadedMixes(userId) {
    ...
    try {
      ...
      if (userMixesError) {
        ...
        // If we can't check due to RLS issues, let's be permissive
        // This prevents blocking legitimate users due to database configuration issues
        console.log("🎯 Allowing application due to database access error");
        return true;
      }
      ...
      if (hasMixes) {
        ...
        console.log("🎯 No mixes found for user");
        
        // TEMPORARY FIX: Let's check if there are any mixes at all in the database
        // This helps us understand if it's a user issue or a database issue
        const { data: allMixes, error: allMixesError } = await supabase
          .from("mixes")
          .select("id, user_id, title")
          .limit(3);
        
        console.log("🎯 Sample of all mixes in database:", { allMixes, allMixesError });
        
        // TEMPORARY BYPASS: For now, let's allow applications even without mixes
        // This prevents blocking users while we debug the mix detection issue
        console.log("🎯 TEMPORARY BYPASS: Allowing application without mix requirement");
        return true;
      }
      
    } catch (error) {
      console.error("🚨 Unexpected error in hasUserUploadedMixes:", error);
      // If we can't determine due to errors, be permissive to avoid blocking users
      console.log("🎯 Allowing application due to unexpected error");
      return true;
    }
  },
