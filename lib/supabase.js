    ...
    try {
      ...
      try {
        ...
        if (!allChecksPass) {
          ...
          if (!audMatch) {
            console.error("  ❌ Bundle ID mismatch - check Apple Developer Console");
            console.error("  ❌ Ensure 'com.rhoodapp.mobile' has Sign in with Apple enabled");
          }
          if (!issMatch) {
            console.error("  ❌ Invalid Apple issuer - token may be corrupted");
          }
          if (!nonceMatch) {
            console.error("  ❌ Nonce mismatch - Supabase will reject this token");
          }
        }
      } catch (debugError) {
        console.log(
