...
export const db = {
  ...
  async findOrCreateIndividualMessageThread(user1Id, user2Id) {
    try {
      ...
      // If we found any threads (from any order), use the first one
      if (anyThreads && anyThreads.length > 0) {
        console.log(`✅ Found existing thread: ${anyThreads[0].id}`);
        return anyThreads[0].id;
      }

      // Try to find an existing thread (this should not be needed now, but keeping as fallback)
