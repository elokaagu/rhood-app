  ...
  async signInWithApple() {
    ...
    try {
      ...
      try {
        ...
        // Temporary TestFlight debugging - show validation results in alert
        const debugMessage = `Apple Sign-In Debug:
        
Bundle ID (aud): ${audMatch ? "✅ PASS" : "❌ FAIL"}
Expected: com.rhoodapp.mobile
Received: ${claims.aud}

Apple Issuer (iss): ${issMatch ? "✅ PASS" : "❌ FAIL"}
Expected: https://appleid.apple.com
Received: ${claims.iss}

Nonce Claim: ${nonceMatch ? "✅ PASS" : "❌ FAIL"}
Expected: ${hashedNonceB64Url.substring(0, 20)}...
Received: ${claims.nonce ? claims.nonce.substring(0, 20) + "..." : "none"}

Overall: ${allChecksPass ? "✅ ALL CHECKS PASS" : "❌ VALIDATION FAILED"}`;

        // Show alert for TestFlight debugging
        Alert.alert("Apple Sign-In Debug", debugMessage, [
          { text: "Continue Anyway", style: "default" },
          {
            text: "Cancel",
            style: "cancel",
            onPress: () => {
              throw new Error("User cancelled due to validation failure");
            },
          },
        ]);
      ...
      // Additional debugging for TestFlight
      const debugSupabaseMessage = `Supabase Call Debug:\n\nRaw Nonce (to Supabase): ${rawNonce.substring(0, 20)}...\nLength: ${rawNonce.length}\n\nHashed Nonce (from Apple): ${hashedNonceB64Url.substring(0, 20)}...\nLength: ${hashedNonceB64Url.length}\n\nToken Length: ${credential.identityToken.length}`;

      Alert.alert("Supabase Call Debug", debugSupabaseMessage, [
