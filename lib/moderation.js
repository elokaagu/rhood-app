...
import { validateBlockTarget } from "./productFlows";
...
export async function blockUser(blockedId) {
  ...
  const row = validateBlockTarget(user?.id, blockedId);
  const { error } = await supabase.from("blocked_users").insert(row);
  if (error && error.code !== "23505") {
    throw error;
  }
  // Guideline 1.2: blocking must also notify the developer.
  try {
    await reportContent({
      targetType: "user",
      targetId: blockedId,
      ...
      reason: "block",
      details: "Blocked by user; hide from their feed immediately.",
    });
  } catch (_e) {
    /* Block still stands if the report insert fails. */
  }
}
...
export function promptReport({ targetType, targetId, targetUserId, onDone }) {
  ...
  if (Platform.OS === "ios") {
    rhoodAlert("Report", "Why are you reporting this?", [
  ...
  rhoodAlert("Report", "Why are you reporting this?", [
    { text: "Spam", onPress: () => send("spam") },
    {
      text: "More…",
      onPress: () => {
        rhoodAlert("Report", "Choose a reason", [
...
export function promptBlockUser({ userId, name, onBlocked }) {
  ...
  rhoodAlert(
    label,
    "They'll disappear from Listen, Discover, and Messages immediately. Our team is notified and aims to review within 24 hours. They won't be told you blocked them.",
    [
      { text: "Cancel", style: "cancel" },
      {
        text: "Block",
        ...
        onPress: async () => {
          try {
            ...
            rhoodAlert("Blocked", "You can still delete your account in Settings if you need to leave R/HOOD.");
            onBlocked?.();
          } catch (error) {
            rhoodAlert("Couldn't block", error?.message || "Please try again.");
          }
        },
      },
    ]
