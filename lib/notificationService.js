...
export async function registerExpoToken(userId, expoToken, deviceId, platform) {
  try {
    // onConflict targets the (user_id, device_id) unique constraint added by
    // database-migrations/fix-user-expo-tokens-upsert-conflict.sql. Without
    // it, Supabase's default upsert conflict target is the primary key —
    // `id`, an auto-generated UUID never in this payload — so it could
    // never match an existing row and every call (every app launch) added a
    // brand-new one instead of refreshing the existing device's row.
