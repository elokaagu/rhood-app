...
export default function AIMatchmakingScreen({ userId, onNavigate }) {
  ...
  const generateAIMatches = async () => {
    if (!aiConfig.apiKey) {
      Alert.alert('Configuration Required', 'Please set your AI API key in settings');
      setShowConfig(true);
      return;
    }
    ...
    try {
      ...
      Alert.alert('AI Matching Error', error.message || 'Failed to generate AI matches');
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateInsights = async () => {
    ...
    try {
      setLoading(true);
      const aiInsights = await aiMatchmaking.generateInsights(userId, aiConfig.apiKey, aiConfig.provider);
      setInsights(aiInsights);
      setShowInsights(true);
    } catch (error) {
      console.error('Error generating insights:', error);
      Alert.alert('Insights Error', error.message || 'Failed to generate AI insights');
    } finally {
      setLoading(false);
    }
  };
    ...
    <View key={match.opportunity_id} style={styles.matchCard}>
      <View style={styles.matchHeader}>
        <View style={styles.matchScoreContainer}>
          <Text style={[styles.matchScore, { color: getMatchScoreColor(match.compatibility_score) }]}>
            {match.compatibility_score}%
          </Text>
          <Text style={styles.matchScoreLabel}>AI Match</Text>
        </View>
        <View style={styles.matchTypeContainer}>
          <Text style={[styles.matchType, { color: getMatchTypeColor(match.match_type) }]}>
          ...
          <ScrollView style={styles.configForm}>
            <View style={styles.inputGroup}>
              ...
              <View style={styles.radioGroup}>
                <TouchableOpacity
                  style={[styles.radioOption, aiConfig.provider === 'openai' && styles.radioSelected]}
                  onPress={() => updateAIConfig({ provider: 'openai' })}
                >
                  <Text style={styles.radioText}>OpenAI (GPT-4)</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.radioOption, aiConfig.provider === 'claude' && styles.radioSelected]}
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.dropdown}>
                ...
                <Ionicons name="chevron-down" size={20} color="hsl(0, 0%, 70%)" />
              </View>
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) => updateAIConfig({ limit: parseInt(text) || 10 })}
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <ScrollView style={styles.insightsContent}>
            {insights && (
              <View>
                <Text style={styles.insightsText}>{JSON.stringify(insights, null, 2)}</Text>
              </View>
            )}
          </ScrollView>
        </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Intelligent DJ-opportunity matching powered by AI</Text>
        ...
        <View style={styles.headerActions}>
          ...
          >
            <Ionicons name="settings-outline" size={20} color="hsl(0, 0%, 100%)" />
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            disabled={loading}
          >
            <Ionicons name="analytics-outline" size={20} color="hsl(0, 0%, 100%)" />
          </TouchableOpacity>
        </View>
      </View>
