-- Add credits system to R/HOOD app (Fixed version)
-- Run this in your Supabase SQL editor

-- First, check if achievements table exists and what columns it has
...
CREATE TABLE IF NOT EXISTS achievements (
    ...
    icon VARCHAR(50),
    category VARCHAR(50),
    credits_value INTEGER DEFAULT 5,
...
CREATE INDEX IF NOT EXISTS idx_user_achievements_credits_awarded ON user_achievements(credits_awarded);
...
-- Update existing achievements with default credit values (using 'name' column instead of 'title')
UPDATE achievements 
...
-- Add some example achievements with credit values (using 'name' column instead of 'title')
