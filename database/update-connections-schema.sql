-- Update Connections Table Schema for Notifications System
-- Run this in your Supabase SQL Editor

-- 1. First, let's check the current connections table structure
...
-- 2. Create new connections table with proper schema
...
-- 3. Migrate existing data if connections table exists
DO $$
BEGIN
  -- Check if old connections table exists and has data
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'connections' AND table_schema = 'public') THEN
    
    -- Check if the old columns exist before trying to migrate
    IF EXISTS (
      ...
      -- Migrate follower_id/following_id format to user_id_1/user_id_2 format
      INSERT INTO connections_new (user_id_1, user_id_2, status, initiated_by, created_at, updated_at, accepted_at)
      SELECT 
        CASE WHEN follower_id < following_id THEN follower_id ELSE following_id END as user_id_1,
        CASE WHEN follower_id < following_id THEN following_id ELSE follower_id END as user_id_2,
        'accepted' as status, -- Assume existing connections are accepted
        follower_id as initiated_by,
        created_at,
        created_at as updated_at,
        created_at as accepted_at
      FROM connections
      WHERE NOT EXISTS (
        SELECT 1 FROM connections_new 
        WHERE (connections_new.user_id_1 = CASE WHEN follower_id < following_id THEN follower_id ELSE following_id END
               AND connections_new.user_id_2 = CASE WHEN follower_id < following_id THEN following_id ELSE follower_id END)
      );
      
      RAISE NOTICE 'Migrated % existing connections from follower_id/following_id format', (SELECT COUNT(*) FROM connections);
    ELSE
      -- Check if the new columns exist (user_id_1, user_id_2)
      IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'connections' 
        ...
        -- Migrate from new format (already has user_id_1, user_id_2)
        INSERT INTO connections_new (user_id_1, user_id_2, status, initiated_by, created_at, updated_at, accepted_at)
        SELECT 
          user_id_1,
          user_id_2,
          COALESCE(status, 'accepted') as status, -- Use existing status or default to accepted
          COALESCE(initiated_by, user_id_1) as initiated_by, -- Use existing initiated_by or default to user_id_1
          created_at,
          COALESCE(updated_at, created_at) as updated_at,
          COALESCE(accepted_at, created_at) as accepted_at
        FROM connections
        WHERE NOT EXISTS (
          SELECT 1 FROM connections_new 
          WHERE (connections_new.user_id_1 = connections.user_id_1
                 AND connections_new.user_id_2 = connections.user_id_2)
        );
        
        RAISE NOTICE 'Migrated % existing connections from user_id_1/user_id_2 format', (SELECT COUNT(*) FROM connections);
      ELSE
        RAISE NOTICE 'No existing connections table found or unrecognized schema - skipping migration';
      END IF;
    END IF;
  ELSE
    RAISE NOTICE 'No existing connections table found - creating fresh table';
  END IF;
END $$;

-- 4. Drop old connections table and rename new one
...
-- 12. Create function to create connection requests
...
-- 13. Create function to accept connection requests
...
-- 17. Create message notification trigger
...
-- 18. Create message notification trigger
...
-- 19. Create function to get unread notification counts
...
-- 20. Create function to get unread message count
