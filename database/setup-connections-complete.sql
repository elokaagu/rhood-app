...
-- It will create tables, add mock users, and create connections
...
RAISE NOTICE '✅ Mock DJ profiles created successfully';

-- ============================================
-- STEP 3: Create connections between DJs
-- ============================================

-- Get the first registered user (likely the main user)
DO $$
DECLARE
  main_user_id UUID;
...
BEGIN
  -- Try to get the main user (exclude our mock users)
  ...
  IF main_user_id IS NOT NULL THEN
    ...
    RAISE NOTICE '✅ Created connections to main user';
  END IF;
